...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  let draftsQuery = context.service.from("content_distribution_drafts").select("id,campaign_id,channel,locale,status,created_at,published_at,scheduled_for,brand_template_id,generation_metadata").limit(5000);
  ...
  const manualRows = drafts.flatMap((draft) => {
    const performance = safeSnapshot(safeSnapshot(draft.generation_metadata).manual_performance);
    const snapshots = safeSnapshot(performance.snapshots);
    return ["d7", "d30"].flatMap((period) => {
      const snapshot = safeSnapshot(snapshots[period]);
      return Object.keys(snapshot).length ? [{ draftId: draft.id, campaignId: draft.campaign_id, channel: draft.channel, period, publicationUrl: String(performance.publication_url || ""), snapshot }] : [];
    });
  });
  const manualFields = ["impressions", "reach", "reactions", "comments", "shares", "saves", "clicks"];
  const manualMedians = Object.fromEntries(["d7", "d30"].map((period) => [period, Object.fromEntries(manualFields.map((field) => [field, median(manualRows.filter((row) => row.period === period).flatMap((row) => typeof row.snapshot[field] === "number" ? [row.snapshot[field]] : []))]))]));
  ...
  return opsData({ filters: { period, channel, locale, status }, page, params: params.toString(), runs: runsPageResult.data || [], runCount: runsPageResult.count,
    ...
    flagCounts: [...flagCounts.values()].sort((a, b) => b.count - a.count), issues, manualRows, manualMedians }, context.headers);
}
...
export default function OpsInsights({ loaderData }: { loaderData: any }) {
  ...
  return <>
    ...
    <section className="ops-panel"><h2>Resultados manuales</h2><p className="ops-muted">Muestra y medianas declaradas por el operador. Son descriptivas: no atribuyen causalidad al contenido.</p>{loaderData.manualRows.length ? <><div className="ops-manual-summary">{(["d7", "d30"] as const).map((period) => <article key={period}><strong>{period.toUpperCase()}</strong><span>{loaderData.manualRows.filter((row: any) => row.period === period).length} observaciones</span><small>Impresiones medianas: {loaderData.manualMedians[period].impressions == null ? "Sin datos" : Number(loaderData.manualMedians[period].impressions).toLocaleString("es-AR")}</small><small>Clics medianos: {loaderData.manualMedians[period].clicks == null ? "Sin datos" : Number(loaderData.manualMedians[period].clicks).toLocaleString("es-AR")}</small></article>)}</div><ul className="ops-metric-list">{loaderData.manualRows.slice(-10).reverse().map((row: any) => <li key={`${row.draftId}-${row.period}`}><StatusBadge value={row.period}/><span>{row.channel} · {row.snapshot.impressions ?? "—"} impresiones · {row.snapshot.clicks ?? "—"} clics</span><Link to={`/ops/social/${row.campaignId}?variant=${row.draftId}`}>Abrir</Link></li>)}</ul></> : <p className="ops-muted">Todavía no hay snapshots D7 o D30.</p>}</section>
